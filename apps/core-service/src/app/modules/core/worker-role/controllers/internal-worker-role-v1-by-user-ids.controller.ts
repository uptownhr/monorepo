...
@Controller('/internal/worker-roles/v1/by-user-ids/:userIds')
@UseGuards(BambeeAuthGuard(TokenType.Service))
export class InternalWorkerRoleByUserIdsV1Controller {
  constructor(private workerRoleService: WorkerRoleService) {}

  @Get()
  public async getAllWorkerRoles(@Param('userIds') userIds: string): Promise<Array<WorkerRoleV1Response>> {
    const res = await this.workerRoleService.getAllWorkerRolesForUserIds(userIds.split(','));
