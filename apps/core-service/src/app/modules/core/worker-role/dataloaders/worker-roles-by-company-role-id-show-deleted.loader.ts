...
export class WorkerRoleByCompanyRoleShowDeletedLoader extends DataLoader<string, WorkerRoleDTO[]> {
  constructor(protected workerRoleRepo: WorkerRoleRepository) {
    super(async (keys: readonly string[]): Promise<ArrayLike<WorkerRoleDTO[] | Error>> => {
      const dtoResults = await this.workerRoleRepo.getWorkerRolesByCompanyRoleId([...keys], true);
